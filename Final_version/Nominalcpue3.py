...
def standard(spid=0, month=4):
    ...
    print(name)
    X_train, X_test, y_train, y_test = train_test_split(f, t, test_size=0.25, random_state=100)
    ...
    p = rf.predict(X_test)
    preRF = pd.DataFrame(np.log(p+1), columns=["RF"])

    rf = XGBRegressor(n_estimators=200, eta=0.02)
    rf.fit(X_train, y_train)
    p = rf.predict(X_test)
    preXGB = pd.DataFrame(np.log(p+1), columns=["XGBoost"])

    rf = GradientBoostingRegressor(n_estimators=150, learning_rate=0.02)
    rf.fit(X_train, y_train)
    p = rf.predict(X_test)
    preGBDT = pd.DataFrame(np.log(p+1), columns=["GBDT"])

    con = pd.concat([X_test["Year"], X_test["Month"],
                     pd.DataFrame(np.array(np.log(y_test+1)), columns=["Nominal CPUE"]),
                     preRF, preXGB, preGBDT], axis=1, ignore_index=False)
    con.to_csv(name+"_concat3.csv")

    print(con.groupby(["Year"]).agg("mean"))
    wid = con.set_index("Year")
    #print(wid)
    sns.lineplot(wid.iloc[:, 1:], markers=True, ci=0)
    plt.title(name)
    plt.ylabel("CPUE(ind./km$^{2}$)")
    #plt.yticks(np.arange(0, int(3*np.mean(t)), int(np.mean(t)/2)))
    plt.savefig("SCPUE" + name + ".svg")
    plt.savefig("SCPUE"+name+".tiff")
    plt.show()
